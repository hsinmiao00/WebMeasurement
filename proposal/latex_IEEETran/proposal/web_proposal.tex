...
\title{Variations in Tracking in Relation to Geographic Location (Project Proposal)}
%
%
...
\IEEEPARstart{I}{n} 
this project, we will be comparing the amount of trackers on websites that operate in various countries with different privacy models. We have chosen France and Germany to represent the comprehensive model, the United States and Japan to represent the sectoral model, Australia and New Zealand to represent the co-regulatory model, and finally Russia and China to represent the mixed/no-policy model. The sites that we are interested in are Alexa Top 500 sites that have domains in multiple countries. For example, we can compare Google.com (US version) with Google.com (German version with no country subdomain) with Google.com.de (German version with country subdomain) and determine if there is a difference in terms of trackers. We will also examine how connecting to these sites from different countries affects tracking.

We plan to locate and identify these trackers using 3rd party HTTP requests and cookies, examining other criteria if necessary. Automation of the process will be handled using the OpenWPM \cite{openwpm} tool which allows for synchronization across browsers and virtual machines ensuring that requests will occur at the same time. A Tor proxy \cite{tor} will be used to allow us to spoof the location we connect from.
...
Privacy laws have been enacted worldwide with the purpose of protecting internet users' private information. Privacy laws can be divided into four main models \cite{IAPPbook} that differ in scope, enforcement, and adjudication: the comprehensive model, the sectoral model, the co-regulatory model, and mixed/no-policy model. These models impact how countries handle privacy both legally and culturally, specifically in the realms of online tracking and privacy legislation. While using the Ghostery browser add-on we discovered that websites utilize a diverse plethora of trackers for various purposes. There is currently a lack of information as to how trackers differ between countries that employ these different models.
...
\begin{enumerate}
	\item 10/24 - 11/4:
	\begin{enumerate}
		\item Set up the Tor network. (Scott)
		\item Set up the OpenWPM platform. (Nathaniel and Hsin) 
		\begin{enumerate}
			\item Run test cases and fully test effectiveness of definitions.
		\end{enumerate}
		\item Obtain the website list from Alexa. (Nathaniel and Hsin) 
		\begin{enumerate}
			\item Compile into sets of websites, broken down by country and analysis type.
		\end{enumerate}
		\item Look into Parallel Data Lab and/or set up appropriate hardware for virtual machines.
	\end{enumerate}
	\item11/4 - 11/25:
	\begin{enumerate}
		\item Write and test OpenWPM script for experiment automation.
		\item Run experiment.
		\item Data analysis.
		\begin{enumerate}
			\item Extract data from OpenWPM databases.
			\item Statistical analysis of data sets.
		\end{enumerate}
	\end{enumerate}
	\item 11/25 - 12/4:
	\begin{enumerate}
		\item Finalize the result.
		\item Design the poster.
		\item Write the final paper.
		\item Extend literature review.
	\end{enumerate}
\item 12/4: Poster fair.
\item 12/12: Final paper.
\end{enumerate}
